...
void solve() {
    ...
    */
    auto ok = [&] (int mid) -> bool {
        vi cnt(m);
        forn(i, m) forn(j, n)  if(a[j][i] >= mid) cnt[i]++;
    ...
    auto safe = [&]() -> int {
        vi mx(n, MIN);
        forn(i, n) forn(j, m) mx[i] = max(mx[i], a[i][j]);
        int mn = MAX;
        forn(i, n) mn = min(mn, mx[i]);
        return mn;
    };

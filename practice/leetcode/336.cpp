...
bool is_palindrome(vector<string>& s, int x, int l, int r) {
    for(int i = 0; l+i < r-i; i++) 
        if(s[x][l+i] != s[x][r-i]) return false;
    return true;
}
...
vector<vector<int>> palindromePairs(vector<string>& w) {
    ...
    //Reverse hashing
    vector<vector<ll>> rh(w.size());
    for(int i = 0; i < w.size(); i++) reverse(w[i].begin(), w[i].end());

    for(int i = 0; i < rh.size(); i++) {
        rh[i].resize(w[i].size()+1), rh[i][0] = 1;
        for(int j = 1; j < rh[i].size(); j++) rh[i][j] = ((rh[i][j-1] * A) + w[i][j-1]) % B;
    }
        ...
        for(int j = i+1; j < w.size(); j++) {
            if(w[i].size() == w[j].size()) {
                ...
                if(w[i].size() == w[j].size()) 
                    ...
                    int ind = (w[i].empty()) ? j : i;
                    if(is_palindrome(w, ind, 0, w[ind].size()-1))
                        ans.push_back({i, j}), ans.push_back({j, i});
                }
            }
            else {
                ...
                if(hash(smol, 0, w[smol].size()-1) == rhash(big, 0, w[smol].size()-1))
                    if(is_palindrome(w, big, w[smol].size(), w[big].size()-1)) ans.push_back({smol, big});
                if(hash(smol, 0, w[smol].size()-1) == rhash(big, w[big].size()-w[smol].size(), w[big].size()-1))
                    if(is_palindrome(w, big, 0, w[big].size()-w[smol].size()-1)) ans.push_back({big, smol});
            }
        }
    }
    return ans;

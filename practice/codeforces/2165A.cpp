...
void solve() {
    ...
    ll sm = 0;
    forn(i, n-2) {
        int one = find(par, (proc[i]-1+n)%n); 
        int two = find(par, (proc[i]+1+n)%n);
        int mn = (a[one] < a[two]) ? one : two;
        int mx = (a[one] < a[two]) ? two : one;
        sm += a[mn];
        cout << proc[i] << ' ' << one << ' ' << two << ' ' << a[mn] << ' ' << mn << '\n';

        par[proc[i]] = mx;
    }
    ll mx = 0;
    forn(i, n) mx = max(mx, (ll) a[i]);
    cout << "Pars: ";
    forn(i, n) cout << par[i] << ' ';
    cout << '\n';
    cout << sm + mx << '\n';

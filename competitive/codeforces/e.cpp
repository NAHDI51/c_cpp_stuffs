...
void solve() {
    ...
    auto ok = [&](int m) -> bool {
        int cnt = 0;
        for (int j = 0; j <= a[0]-m && v.sz() < k; j++) v.pb(j);
        for(int i = 0; i < n-1 && v.sz() < k; i++) {
            int L = a[i]+m, R = a[i+1]-m;
            for(int j = L; j <= R && v.sz() < k; j++) v.pb(j);
        }
        for(int j = a[n-1]+m; j <= x && v.sz() < k; j++) v.pb(j);
        return v.sz() >= k;
    };
    vi anss;
    int lo = 0, hi = x, ans = -1;

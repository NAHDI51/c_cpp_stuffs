    ...
    while (i < nn) {
        if (s[i] == '-') {
            ...
            while( (i < nn) && (s[i] != '+') && (s[i] != '-') ) {
                ...
                if(mlt2 == 0) mlt2 = 1;
                for(int j = 0; j < (mult*mlt2); j++) {
                    if(in) vin.push_back(c);
                    else vout.push_back(c);
                    // print(c);
                }
            }
        }
...
int32_t solve2(int32_t testNo) {
    ...
    f0(i,n) {
        f0(j,m) {
            ...
            // print(i,j);
            lr = 1;
            while( ((j+lr) < (m)) && ( ((j-lr) >= 0) && ((i+lr+lr) < (n)) ) ) {
                // print(lr);
                if((arr[i+lr][j+lr] != '#') || (arr[i+lr][j-lr] != '#')) {na = false; break;}
                if(arr[i+lr+1][j+lr-1] == '#') {
                    if (arr[i+lr+1][j-lr+1] == '#') {
                        break;
                    } else {
                        na = false;
            ...
            if(na) {
                if((i+lr+lr) >= n) continue;
                // if(lr == 1) continue;
                int lr2 = 0;
                bool ya = true;
                for (lr2 = 1; (lr2 <= lr) && ya;lr2++) {
                    for(int x = (j-lr2+1); x <= (j+lr2-1); x++) {
                        if(arr[i+lr2][x] == '#') {
                            ya = false;
                            break;
                        }
                ...
                for (lr2 = 1; (lr2 <= lr) && ya;lr2++) {
                    if((arr[i+lr+lr-lr2][j-lr2] != '#') || (arr[i+lr+lr-lr2][j+lr2] != '#')) {
                        ya = false;
                        break;
                    } else
                    for(int x = (j-lr2+1); x <= (j+lr2-1); x++) {
                        if(arr[i+lr+lr-lr2][x] == '#') {
                            ya = false;
                            break;
                        }
                    }
                }
                if(arr[i+lr+lr][j] != '#') ya = false;
                if(ya){
                    ans++;
                    // print(i,j,lr);
                }
